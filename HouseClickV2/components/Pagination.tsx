import {
  ...
  export default function Pagination({ num_pages, selected, onChange }: PaginationProps) {
    const pages = [...Array(Math.min(num_pages, 10)).keys()].map(page => {
      return (
        <a 
          ...
          className={`inline-flex items-center border-t-2 border-transparent px-4 p-2 text-sm font-medium text-primary-300 hover:underline ${page + 1 === selected ? 'active-page': ''}`}
        > 
          {page+1}  
        </a>
      );
        ...
        <div className="-mt-px flex w-0 flex-1">
          {
            selected > 1 ? (
              <a 
                href="#"  
                className="inline-flex items-center border-t-2 border-transparent pr-1 pt-4 text-sm font-medium text-primary-300 hover:underline" 
                onClick={(event: React.MouseEvent<HTMLAnchorElement>) => {
                  event.preventDefault();
                  onChange(selected-1);
                }}
