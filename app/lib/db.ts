...
function convertRowToListing(row: ListingRow): Listing {
  return {
    ...
    description: row.description.substring(0, row.description.lastIndexOf('.') + 1),
    price_formatted: `£${row.price.toLocaleString('en-US')}`,
    price: row.price,
...
export async function search(query: SearchParams) {
  ...
  let baseQuery = `SELECT id, type, price, town, district, postcode1, postcode2, duration, date, urls, description, title, rooms, bathrooms, sold FROM uk_house_listings`;
  const conditions = [];
  let values = [];
  if (query.searchTerm !== '') {
  ...
  if (query.searchTerm !== '') {
    baseQuery +=` ORDER BY ts_rank(to_tsvector(title || ' ' || description), to_tsquery($1)) DESC, ${query.sort.column} ${query.sort.ascending ? 'ASC' : 'DESC'} LIMIT 20 OFFSET 0`;
  } else {
    baseQuery += ` ORDER BY ${query.sort.column} ${query.sort.ascending ? 'ASC' : 'DESC'} LIMIT 20 OFFSET ${from}`;
  }
