...
export default function PriceOverTime({ data, stack, fill, onSelect, metadata, debug  }) {
    const [loading, setLoading] = useState(true);

    const xAxis = Array.from(new Set(data.map((p) => p.month)));

    let values = {};
    values['National average'] = { name: "National average", data: [] };
    values['Area average'] = { name: "Area average", data: [] };
    data.forEach((p) => {
        values['National average'].data.push(p.avg);
        values['Area average'].data.push(p.filter_price);
    })
    ...
    const options = {
        ...
        legend: {
            ...
            bottom: '5%',
            // data: ["Area price", "National price"]
        },
        yAxis: {
            splitLine: {
                show: true,
