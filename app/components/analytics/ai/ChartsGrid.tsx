"use client"
import GenericChart, { ChartProps, DataPoint } from "@/components/analytics/ai/GenericChart";
...
function DynamicGrid({ charts }: GridProps) {
    return (
        charts.map((chart, index) => (
            <div className="flex flex-col gap-4" key={index}>
                <p className="text-[#FFF] whitespace-nowrap overflow-hidden text-overflow-ellipsis text-(length:--typography-font-sizes-1,20px) leading-[150%] font-bold font-inter">{chart.title}</p>
                <GenericChart {...chart} />
            </div>))
    )
